    ...
    return (
      <div className="grid gap-4 sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-3 lg:gap-10 mt-10">
        {ProductList.map((item) => (
          <Card key={item.id} className='card-no-border w-80 md:w-full lg:w-full'>
            <div className='flex items-center gap-1 group p-4'>
              {[...Array(5)].map((_, index) => (
            ...
            <CardFooter className='flex items-center pt-4 px-4'>
              <div className='flex items-center'>
                <img src={item.imageSrc} alt={item.name} className='w-10 h-10 rounded-full object-cover'/>
                <div className="flex flex-col items-start ml-3">
                  <span className="font-semibold text-[15px]">{item.name}</span>
                  <span className="text-sm text-gray-600">{item.profession}</span>
                </div>
              </div>
            </CardFooter>
          </Card>
